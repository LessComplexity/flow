#!/bin/bash
# Full local matmul comparison: Mapal against every CPU baseline, 512..4096, f32+f64.
#
# COMPUTE-ONLY on both sides. The Mapal legs bracket their kernel map with the
# `time` builtin and print `iter ms=` (S30 — MAPAL_PERF is retired here; it timed
# all of mapal_main, data generation included, while every baseline times the
...
emit() { ( cd "$ROOT" && cargo run -q --release -p mapal-backend-llvm --example emit -- "$1" - --rewrite "${@:2}" ); }
mapal_ms() { # <binary> ; min-of-RUNS of the program's own `iter ms=`
  local best="" v
...
for N in $SIZES; do
  for W in f32 f64; do
    SUF=""; [ "$W" = f32 ] && SUF="_f32"
    SRC="$ROOT/benches/matmul/matmul${N}_cap${SUF}.mapal"
    [ -f "$SRC" ] || continue
    ...
    printf '%-6s %-5s %-16s %12s\n' "$N" "$W" "mapal-conf-par" "$(mapal_ms "$TMP/${N}${W}_conf")"
    printf '%-6s %-5s %-16s %12s\n' "$N" "$W" "mapal-fma-par"  "$(mapal_ms "$TMP/${N}${W}_fma")"
    printf '%-6s %-5s %-16s %12s\n' "$N" "$W" "mapal-conf-1t"  "$(MAPAL_PAR=1 mapal_ms "$TMP/${N}${W}_conf")"
    printf '%-6s %-5s %-16s %12s\n' "$N" "$W" "mapal-fma-1t"   "$(MAPAL_PAR=1 mapal_ms "$TMP/${N}${W}_fma")"

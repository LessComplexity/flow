...
__global__ void k0_0(float* result, float* arr, int64_t idx, unsigned int* trap) {
  if (idx < 0 || idx >= (int64_t)4096) { *trap = 2u; return; }
...
__global__ void k2_0(int32_t* out, long long n) {
  long long i = (long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) out[i] = (int32_t)i;
}
__global__ void k2_1(float* out, int32_t* in) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 4096ULL) {
    out[i] = fn3(in[i]);
  }
}
__global__ void k2_2(float* out, int32_t* in) {
  unsigned long long i = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < 4096ULL) {
    out[i] = fn4(in[i]);
  }
}
__global__ void k2_3(float* result, float* arr, int64_t idx) {
  *result = arr[(unsigned long long)idx];
}
...
static float fn0(FlowProd_floatp_floatp_int32_t_int32_t in) {
  ...
  o6.f0 = 0;
  o6.f1 = 0e0f;
  o13 = (int32_t)((uint32_t)in.f2 * (uint32_t)64);
  o7 = o6;
  while (true) {
    o10.f1 = 64;
    o8 = o7.f0;
...
static float* fn1(FlowProd_floatp_floatp in) {
  ...
  while (true) {
    ...
    o15 = fn0(o14);
    o16.f2 = o15;
    cu_check(cudaMalloc((void**)&o17, sizeof(float) * 4096ULL), "cudaMalloc(o17)");
    k1_0<<<(unsigned int)((4096ULL + 255ULL) / 256ULL), 256>>>(o17, o6, (int64_t)o7, o15, d_trap);
    trap_check_after_launch();
    o20.f0 = o17;
    o21.f0 = o20;
    o5 = o21.f0;
...
static void flow_main() {
  ...
  float* t0 = nullptr;
  float* t1 = nullptr;
  cu_check(cudaMalloc((void**)&arena0, 49664ULL), "cudaMalloc(arena0)");
  o2 = (int32_t*)(arena0 + 0ULL);
  k2_0<<<(unsigned int)((4096ULL + 255ULL) / 256ULL), 256>>>(o2, 4096);
  o3 = (float*)(arena0 + 16384ULL);
  k2_1<<<(unsigned int)((4096ULL + 255ULL) / 256ULL), 256>>>(o3, o2);
  o4 = (float*)(arena0 + 32768ULL);
  k2_2<<<(unsigned int)((4096ULL + 255ULL) / 256ULL), 256>>>(o4, o2);
  o5.f0 = o3;
  o5.f1 = o4;
  o6 = fn1(o5);
  t0 = (float*)(arena0 + 49152ULL);
  k2_3<<<1, 1>>>(t0, o6, (int64_t)0);
  cu_check(cudaMemcpy(&o8, t0, sizeof(float), cudaMemcpyDeviceToHost), "cudaMemcpy(index)");
  t1 = (float*)(arena0 + 49408ULL);
  k2_3<<<1, 1>>>(t1, o6, (int64_t)4095);
  cu_check(cudaMemcpy(&o12, t1, sizeof(float), cudaMemcpyDeviceToHost), "cudaMemcpy(index)");
  o9 = o8;
  o13 = o12;
  flow_print_f32(o9, true);

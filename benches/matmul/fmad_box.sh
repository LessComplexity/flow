...
# Builds the MAPAL_PERF kernel binaries with the NEW default (-fmad=true),
...
# naive_cuda.cu, cublas_gemm.cu, mapal_rt.rs, this script.
set -e
cd /root/bench
...
for n in 512 1024 2048 4096; do
  for v in cap_perf cap_f32_perf; do
    nvcc -std=c++17 -fmad=true -arch=sm_89 "matmul${n}_${v}.cu" libmapal_rt.a -o "mm_${v}_$n" -lpthread -ldl -lm
  done
done
nvcc -O3 -arch=sm_89 naive_cuda.cu -o naive_cuda
nvcc -O3 -arch=sm_89 cublas_gemm.cu -lcublas -o cublas_gemm
echo "== mapal kernels, -fmad=true (3 runs each; MAPAL_PERF lines are the data) =="
for n in 512 1024 2048 4096; do
  for v in cap_perf cap_f32_perf; do

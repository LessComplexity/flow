...
# S20c trap-free kernels), (2) FLOW_PERF re-measure, (3) the full v2
...
cargo test -p flow-backend-cuda 2>&1 | tee /root/differential.log | grep -E "test result|FAILED|running" | tail -12

echo "== bench dir setup =="
mkdir -p /root/bench
cp -r benches/matmul/* /root/bench/
cp crates/flow-rt/src/lib.rs /root/bench/flow_rt.rs
cd /root/bench
echo "== builds (runner.sh: flow-rt staticlib, cu/ll legs, baselines, chapel) =="

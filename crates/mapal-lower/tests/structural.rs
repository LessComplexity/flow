...
use mapal_ir::{CategoryIr, FuncKind, ObjectKind, Operation};
...
fn func_named(ir: &CategoryIr, name: &str) -> mapal_ir::FuncId {
    ir.funcs()
...
fn sum_to_n_loop_shape() {
    ...
    let exit = ir
        ...
        .unwrap()
        .1;
    let cond_of = |route: mapal_ir::ObjectId| -> mapal_ir::ObjectId {
...
fn abs_phi_and_neg_literal() {
    ...
    let has_neg_one = ir.objects().any(|(_, o)| {
        o.kind == ObjectKind::Constant && matches!(&o.value, Some(mapal_ir::Value::I32(-1)))
    });
...
fn sepia_map_fold_kinds() {
    ...
    let expected = mapal_ir::Ty::Tuple(vec![
        mapal_ir::Ty::f32(),
        mapal_ir::Ty::Array {
            elem: Box::new(mapal_ir::Ty::Struct {
                name: "Pixel".into(),
                fields: vec![
                    ("r".into(), mapal_ir::Ty::f32()),
                    ("g".into(), mapal_ir::Ty::f32()),
                    ("b".into(), mapal_ir::Ty::f32()),
                ],
            }),
            size: 16,
        },
...
fn fanout_main_print_token_order() {
    ...
    let prints: Vec<&mapal_ir::Morphism> = def
        .morphisms
        .iter()
...
fn countdown_token_bearing_loop() {
    ...
    assert!(
        mapal_ir::ty_contains_token(&merge.ty),
    ...
    let exit = ir
        ...
        .1;
    assert!(
        mapal_ir::ty_contains_token(&ir.object(exit.target).unwrap().ty),
...
fn declared_signatures_match_table() {
    ...
    assert_eq!(ir.object(f.input).unwrap().ty, mapal_ir::Ty::i32());
    assert_eq!(ir.object(f.output).unwrap().ty, mapal_ir::Ty::i32());
    let main = ir.func(func_named(&ir, "main")).unwrap();
    assert_eq!(ir.object(main.input).unwrap().ty, mapal_ir::Ty::IoToken);
    assert_eq!(ir.object(main.output).unwrap().ty, mapal_ir::Ty::IoToken);
}
...
"#;
    ...
    let pair_ty = mapal_ir::Ty::Tuple(vec![mapal_ir::Ty::IoToken, mapal_ir::Ty::f64()]);
...
"#;
    ...
    let main = func_named(&ir, "main");
    let def = ir.func(main).unwrap();
    let times: Vec<&mapal_ir::Morphism> = def
        .morphisms
        .iter()
        .map(|&m| ir.morphism(m).unwrap())
...
"#;
    ...
    let carries_array = |ty: &mapal_ir::Ty| -> bool {
        match ty {
            mapal_ir::Ty::Array { .. } => true,
            mapal_ir::Ty::Tuple(cs) => cs.iter().any(|c| matches!(c, mapal_ir::Ty::Array { .. })),
            _ => false,
        }
    };
    assert!(
...
"#;
    ...
    assert!(
        !mapal_ir::ty_contains_token(&ir.object(upd.source).unwrap().ty),
        "Update source (the (arr, idx, val) triple) must be token-free"
    );
    assert!(
        !mapal_ir::ty_contains_token(&ir.object(upd.target).unwrap().ty),
        "Update result (the fresh array) must be token-free"
    );
    for &mid in ir.in_edges(upd.source).iter() {
        let m = ir.morphism(mid).unwrap();
        assert!(
            !mapal_ir::ty_contains_token(&ir.object(m.source).unwrap().ty),

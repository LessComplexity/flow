//! Runnable dev tool: dump a `.mapal` file's sealed Category-IR as Mermaid.
//!
//! ```text
//! cargo run -p mapal-lower --example dump_ir -- path/to/file.mapal
//! ```
//!
//! Runs the real frontend (`mapal_syntax::parse` → `mapal_lower::lower`) and prints
//! a lint-clean Mermaid flowchart of the sealed [`mapal_ir::CategoryIr`] to stdout.
//! Parse/lower diagnostics (with `line:col`) go to stderr and set a non-zero exit.
//!
//! No interpreter required — this is the `mapal dump-ir --mermaid` slice, available
//! before `mapal-cli` exists (the categorical pipeline `parse ; lower ; to_mermaid`
//! is three calls; see `docs/architecture/categorical-model.md` §3). It is a dev
//! example, not the user-facing CLI: rendering here is minimal (mapal-cli owns real
...
fn main() -> ExitCode {
    ...
    let Some(path) = std::env::args().skip(1).find(|a| !a.starts_with('-')) else {
        eprintln!("usage: cargo run -p mapal-lower --example dump_ir -- <file.mapal>");
    ...
    match mapal_lower::lower(&source, &parsed.program) {
        Ok(ir) => {
            let mermaid = ir.to_mermaid();
            // Defense-in-depth: every dump must lint clean (ir DESIGN §14 / D9). A
            // finding here is a mapal-ir bug, not a user error — surface it, still print.
            for finding in mapal_ir::lint_mermaid(&mermaid) {
                eprintln!("dump_ir: WARNING mermaid lint (a mapal-ir bug): {finding}");
            }
            println!("{mermaid}");
            ExitCode::SUCCESS
        }
        Err(diags) => {
            ...
            eprintln!(
                "dump_ir: {} lower error(s) — rejected (out of Mapal-Core, or ill-typed).",
                diags.len()
            );
            ExitCode::FAILURE
        }

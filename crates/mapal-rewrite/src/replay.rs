...
fn reconstruct_loop(
    ...
    // mapal-ir's loop_plan). `replay` is gated by `is_canonical`, so `Some` here.
    let lp = ir
...
fn reconstruct_temp(
            ...
            .all(|&m| matches!(ir.morphism(m).unwrap().op, Operation::Pair { .. }))
    {
        ...
        let new = match &obj.ty {
            mapal_ir::Ty::Tuple(_) => fb.pack(&comps, dest, obj.loc),
            mapal_ir::Ty::Struct { .. } => fb.pack_struct(obj.ty.clone(), &comps, dest, obj.loc),
            mapal_ir::Ty::Array { .. } => fb.pack_array(&comps, dest, obj.loc),
            ...
            .all(|&w| matches!(ir.morphism(w).unwrap().op, Operation::Pair { .. }))
    {
        ...
        return match ret_dest {
            RetDest::Fresh(name) => {
                ...
                match ty {
                    mapal_ir::Ty::Tuple(_) => fb.pack(&comps, Dest::Fresh(name), call_loc),
                    mapal_ir::Ty::Struct { .. } => {
                        fb.pack_struct(ty.clone(), &comps, Dest::Fresh(name), call_loc)
                    }
                    mapal_ir::Ty::Array { .. } => {
                        fb.pack_array(&comps, Dest::Fresh(name), call_loc)
                    }
...
fn back_morph_loc(ir: &CategoryIr, merge: ObjectId) -> mapal_ir::SourceLoc {
    ir.in_edges(merge)
        .iter()
        .find_map(|&m| {
            let mo = ir.morphism(m).unwrap();
...
///
/// Delegates the per-merge attribution to [`mapal_ir::CategoryIr::loop_plan`]
...
/// source components with [`mapal_ir::FnBuilder::map_captured`]; the fused body's
/// capture count is `k`.
#[allow(clippy::too_many_arguments)]
fn emit_fused_map(
    fb: &mut mapal_ir::FnBuilder<'_>,

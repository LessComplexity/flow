...
use mapal_interp::{Outcome, RunResult, TrapKind, eval_call, run};
use mapal_ir::{CategoryIr, Dest, FuncKind, IrBuilder, Operation, SourceLoc, Ty, Value, validate};
use mapal_rewrite::{PassId, RewriteResult, is_canonical, rewrite, rewrite_with};
...
fn RValue_f32(x: f32) -> mapal_interp::RValue {
    mapal_interp::RValue::Scalar(Value::F32(x))
}

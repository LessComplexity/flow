...
//!
//! For each of the 10 in-Core examples (all `examples/*.mapal` except
//! `vector.mapal`, the out-of-Core generics sketch): `parse → lower →
...
use mapal_interp::{RunResult, run};
use mapal_ir::{
    CategoryIr, Dest, FuncKind, IrBuilder, Operation, SourceLoc, Ty, Value, lint_mermaid, validate,
};
use mapal_rewrite::{ReplayError, RewritePlan, is_canonical, replay, rewrite};

const BUDGET: u64 = 100_000;
const L: SourceLoc = SourceLoc { start: 0, end: 0 };

/// The 10 in-Core examples (every `examples/*.mapal` except `vector.mapal`).
const EXAMPLES: &[&str] = &[
    "abs",
    "calc",
    "fanout",
...
/// Parse + lower one `examples/*.mapal` into a sealed IR (asserting clean).
fn build(name: &str) -> CategoryIr {
    ...
    mapal_lower::lower(&src, &po.program)
        .unwrap_or_else(|ds| panic!("{name}: lower failed: {ds:?}"))
}
...
fn update_graph_round_trips_through_rewrite() {
    ...
    let before = mapal_interp::eval_call(&ir, f, arg.clone(), BUDGET);
    ...
    let after = mapal_interp::eval_call(&res.ir, res.ir.entry(), arg, BUDGET);
    assert_eq!(
        after, before,
    ...
    assert_eq!(
        ...
        mapal_interp::Outcome::Done(mapal_interp::RValue::Array(vec![
            ...
            mapal_interp::RValue::Scalar(Value::I32(3)),
        ]))
    );
}
...
/// `crates/mapal-ir/tests/algos.rs::nested_loops_multi_merge_one_scc`.

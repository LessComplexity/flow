//! `mapal-rt` — the shared runtime seam (backend-llvm DESIGN §1, ADR-0020 §2).
//!
...
//! shortest-round-trip `Display`, exactly as `mapal_interp::render` does (both
//! call the same formatter). `mapal_trap` writes the defined message to stderr
...
/// Define one `mapal_print_<ty>(v, newline)` extern. `Display` for every scalar
...
print_fn!(mapal_print_i32, i32);
print_fn!(mapal_print_i64, i64);
print_fn!(mapal_print_u8, u8);
print_fn!(mapal_print_bool, bool);
print_fn!(mapal_print_f32, f32);
print_fn!(mapal_print_f64, f64);
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_print_str(ptr: *const u8, len: usize, newline: bool) {
...
#[unsafe(no_mangle)]
pub extern "C" fn mapal_trap(kind: u32) -> ! {
    let msg = match kind {
        0 => "div_zero",
        1 => "index_oob",
...
///
/// The emitter owns this handle from `mapal_par_begin` through exactly one
/// `mapal_par_finish`; all registration happens before `mapal_par_launch`.
#[repr(C)]
...
/// Experiment lever: override every dispatch's slice size (`MAPAL_SLICE=<elems>`).
///
...
/// knob to search. `MAPAL_PAR`'s sibling, with the same status.
fn slice_override() -> i64 {
    static OVERRIDE: OnceLock<i64> = OnceLock::new();
    *OVERRIDE.get_or_init(|| {
        std::env::var("MAPAL_SLICE")
            .ok()
...
fn thread_count(mapal_par: Option<&str>, available: usize, quota: Option<usize>) -> usize {
    mapal_par
...
fn configured_threads() -> usize {
    let mapal_par = std::env::var("MAPAL_PAR").ok();
    let available = thread::available_parallelism()
        .map(usize::from)
        .unwrap_or(1);
    thread_count(mapal_par.as_deref(), available, cgroup_quota())
}
...
#[unsafe(no_mangle)]
pub extern "C" fn mapal_perf_begin() {
...
#[unsafe(no_mangle)]
pub extern "C" fn mapal_perf_end() {
    let elapsed = lock(&PERF_START)
        .take()
        .expect("mapal_perf_end called without mapal_perf_begin")
        .elapsed()
        .as_secs_f64()
        * 1000.0;
    emit(&format_args!("MAPAL_PERF total ms={elapsed:.4}"), true);
}

/// The process-lifetime monotonic epoch for `mapal_time_ms` (plan-time-builtin):
...
/// monotonic clock (`std::time::Instant`) as `mapal_perf_begin`/`mapal_perf_end`,
/// measured against the process-lifetime [`TIME_EPOCH`].
#[unsafe(no_mangle)]
pub extern "C" fn mapal_time_ms() -> f64 {
    TIME_EPOCH
...
/// by [`mapal_rt_alloc`], kept as `(address, layout)` so [`mapal_rt_free_all`]
...
/// last reader (`mapal_par_finish` / fn end). Ceiling: a program that wanted
/// per-allocation lifetimes, or heap-lowered a repeatedly-called fn, needs
/// `mapal_rt_free(ptr)` + an emitter-side last-use point instead.
...
/// handle OOM; `mapal_trap`'s exit-101 contract is for *language* traps).
///
/// The block comes back **resident**: see [`reside`] for why that is part of
/// the contract and not an optimisation (plan-s33 CR-1).
#[unsafe(no_mangle)]
pub extern "C" fn mapal_rt_alloc(bytes: i64, align: i64) -> *mut u8 {
    let layout = Layout::from_size_align(bytes as usize, align as usize)
        .expect("mapal_rt_alloc: emitter passes a valid size/alignment pair");
    ...
    assert!(layout.size() > 0, "mapal_rt_alloc: zero-sized block");
...
/// morphism, and the reason `mapal_rt_alloc`'s result is a *materialised* block
...
/// point that can read arena memory (post-`mapal_par_finish` in the parallel
/// flavor, immediately before `ret` otherwise).
#[unsafe(no_mangle)]
pub extern "C" fn mapal_rt_free_all() {
...
impl Pool {
    ...
    fn with_workers(threads: usize, workers: usize) -> Arc<Self> {
        ...
        // MAPAL_PAR=1 is the thread-free sequential lever. With a real pool the
        ...
        let terminal = {
            ...
            if task.slices_left != 0 {
                ...
                for dependent in dependents {
                    ...
                    // them via mapal_par_run_pinned; the epoch bump below wakes
...
/// `mapal_par_finish`.
#[unsafe(no_mangle)]
pub extern "C" fn mapal_par_begin(n_tasks: u32) -> *mut FlowParRun {
...
///
/// # Safety
/// `handle` must be a live handle from `mapal_par_begin`, and every task index
/// must be registered exactly once before launch.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_task(
...
/// still be open before `mapal_par_launch`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_dep(handle: *mut FlowParRun, before: u32, after: u32) {
    let run = &unsafe { run_handle(handle) }.run;
    let mut state = lock(&run.state);
...
/// and `frame` must remain valid through `mapal_par_finish`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_launch(handle: *mut FlowParRun, frame: *mut u8) {
...
/// the host spine executes it via `mapal_par_run_pinned` at its topo position.
///
/// # Safety
/// `handle` must be live, `idx` in range, and registration still open.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_pin(handle: *mut FlowParRun, idx: u32) {
    let run = &unsafe { run_handle(handle) }.run;
    let mut state = lock(&run.state);
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_wait(handle: *mut FlowParRun, entries: *const u64, len: u32) {
    let run = Arc::clone(&unsafe { run_handle(handle) }.run);
...
fn check_trap(run: &Run, topo: i64) {
    ...
    if trap != 0 && ((trap >> 32) as i64) < topo {
        mapal_trap((trap as u32) - 1);
    }
}

/// Observe the run's deterministic trap flag at an oracle checkpoint.
///
/// A recorded trap with `topo_idx < topo` is delivered through `mapal_trap`;
/// `i64::MAX` therefore checks every representable task trap.
///
/// # Safety
/// `handle` must be live and launched.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_check(handle: *mut FlowParRun, topo: i64) {
...
#[unsafe(no_mangle)]
pub extern "C" fn mapal_par_trap(topo: i64, kind: u32) {
    assert!(
    ...
    CURRENT_RUNS.with(|runs| {
        ...
        let current = runs
            .last()
            .expect("mapal_par_trap called outside a parallel task");
...
/// `topo`.
#[unsafe(no_mangle)]
pub extern "C" fn mapal_par_watermark(topo: i64) {
    assert!(
        (0..u32::MAX as i64).contains(&topo),
    ...
    let current = CURRENT_RUNS.with(|runs| {
        *runs
            .borrow()
            .last()
            .expect("mapal_par_watermark called outside a parallel task")
    });
    let run = unsafe { &*current.run };
    let mut state = lock(&run.state);
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_run_pinned(handle: *mut FlowParRun, idx: u32) {
    let run = Arc::clone(&unsafe { run_handle(handle) }.run);
    ...
    let (def, ranges) = {
        ...
        assert!(task.pinned, "mapal_par_run_pinned on an unpinned task");
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn mapal_par_finish(handle: *mut FlowParRun) {
    assert!(!handle.is_null(), "null flow parallel run");
...
mod tests {
    ...
    fn arena_alloc_is_usable_and_freed() {
        let a = mapal_rt_alloc(4096, 64);
        let b = mapal_rt_alloc(4096, 64);
        let big_bytes = 1024 * 1024 + 1; // > 1 page, not a page multiple
        let c = mapal_rt_alloc(big_bytes as i64, 64);
    ...
    /// Render-parity table (DESIGN §1): every scalar mapal-rt prints must be
    /// byte-identical to `mapal_interp::render`. mapal-rt prints `v` via
    ...
    fn render_parity() {
        macro_rules! check {
            ($disp:expr, $val:expr) => {{
                let rt = format!("{}", $disp); // what mapal-rt's emit() writes
        ...
        // Str — mapal_print_str renders bytes via Display (== render(Str) == s).
    ...
    fn split_covers_each_index_once() {
        ...
        unsafe {
            mapal_par_task(handle, 0, 1, hit_slice, n as i64, 0, 0, 0, 0);
            mapal_par_launch(handle, frame_ptr(&*frame));
            mapal_par_finish(handle);
        }
        ...
        unsafe {
            mapal_par_task(handle, 0, 1, zero_split, 0, 0, 0, 0, 0);
            mapal_par_task(handle, 1, 0, zero_dependent, 0, 0, 0, 0, 0);
            mapal_par_dep(handle, 0, 1);
            mapal_par_launch(handle, frame_ptr(&*zero));
            mapal_par_finish(handle);
        }
    ...
    fn dependent_starts_after_all_slices_complete() {
        ...
        let handle = test_run(2, 4);
        unsafe {
            mapal_par_task(handle, 0, 1, dep_producer, n as i64, 0, 0, 0, 0);
            mapal_par_task(handle, 1, 0, dep_consumer, 0, u32::MAX, 0, 0, 0);
            mapal_par_dep(handle, 0, 1);
            mapal_par_launch(handle, frame_ptr(&*frame));
            mapal_par_finish(handle);
        }
    ...
    fn inline_runs_ready_tasks_by_ascending_index() {
        ...
        unsafe {
            mapal_par_task(handle, 2, 0, record_order, 12, 100, 0, 0, 0);
            mapal_par_task(handle, 0, 0, record_order, 10, 0, 0, 0, 0);
            mapal_par_task(handle, 1, 0, record_order, 11, 50, 0, 0, 0);
            mapal_par_launch(handle, frame_ptr(&*frame));
        }
        assert!(lock(&frame.calls).is_empty(), "launch must only seed");
        unsafe { mapal_par_finish(handle) };
    ...
    extern "C" fn trap_after_barrier(_lo: i64, hi: i64, frame: *mut u8) {
        ...
        mapal_par_trap(hi, u32::from(hi == 7));
    }
    ...
    fn trap_flag_keeps_minimum_topo() {
        ...
        unsafe {
            mapal_par_task(handle, 0, 0, trap_after_barrier, 50, 0, 0, 0, 0);
            mapal_par_task(handle, 1, 0, trap_after_barrier, 7, 0, 0, 0, 0);
            mapal_par_launch(handle, frame_ptr(&*frame));
            let entries = [completion_entry(0), completion_entry(1)];
            mapal_par_wait(handle, entries.as_ptr(), entries.len() as u32);
    ...
    fn trapped_task_completes_and_unlocks_its_dependent() {
        ...
        unsafe {
            mapal_par_task(handle, 0, 0, trapping_task, 0, 0, 0, 0, 0);
            mapal_par_task(handle, 1, 0, dependent_after_trap, 0, 0, 0, 0, 0);
            mapal_par_dep(handle, 0, 1);
            mapal_par_launch(handle, frame_ptr(&*frame));
            let entries = [completion_entry(1)];
            mapal_par_wait(handle, entries.as_ptr(), 1);
            assert!(frame.trapping_completed.load(Ordering::SeqCst));
            assert_eq!(frame.dependent_calls.load(Ordering::SeqCst), 1);
    ...
    fn watermark_wait_can_finish_before_task_completion() {
        ...
        unsafe {
            mapal_par_task(handle, 0, 0, publish_then_wait, 0, 0, 0, 0, 0);
            mapal_par_launch(handle, frame_ptr(&*frame));
        }
        ...
        unsafe { mapal_par_wait(handle, below.as_ptr(), 1) };
        ...
        let waiter = thread::spawn(move || {
            ...
            unsafe { mapal_par_wait(handle_addr as *mut FlowParRun, above.as_ptr(), 1) };
            done_tx.send(()).unwrap();
        });
    ...
    fn pinned_task_runs_inline_and_unlocks_dependents() {
        ...
        unsafe {
            mapal_par_task(handle, 0, 1, pinned_slice, n, 0, 0, 0, 0);
            mapal_par_task(handle, 1, 0, pinned_dependent, 0, 0, 0, 0, 0);
            mapal_par_dep(handle, 0, 1);
            mapal_par_pin(handle, 0);
            mapal_par_launch(handle, frame_ptr(&*frame));
            mapal_par_run_pinned(handle, 0);
            let entries = [completion_entry(1)];
            mapal_par_wait(handle, entries.as_ptr(), 1);
            mapal_par_finish(handle);
        }
    ...
    /// it regardless of timing; only mapal_par_run_pinned executes it.
    #[test]
    fn pinned_zero_dep_task_is_never_pool_seeded() {
        ...
        unsafe {
            mapal_par_task(handle, 0, 0, pinned_slice, 1, 0, 0, 0, 0);
            mapal_par_task(handle, 1, 0, pinned_dependent, 0, 0, 0, 0, 0);
            mapal_par_pin(handle, 0);
            mapal_par_launch(handle, frame_ptr(&*frame));
    ...
    fn wait_helps_while_the_background_worker_is_busy() {
        ...
        unsafe {
            mapal_par_task(blocker_run, 0, 0, blocking_task, 0, 0, 0, 0, 0);
            mapal_par_launch(blocker_run, frame_ptr(&*blocker));
        }
        ...
        unsafe {
            mapal_par_task(helped_run, 0, 0, helped_task, 0, 0, 0, 0, 0);
            mapal_par_launch(helped_run, frame_ptr(&*help));
            let entries = [completion_entry(0)];
            mapal_par_wait(helped_run, entries.as_ptr(), 1);
        }

        *lock(&blocker.released) = true;
        blocker.released_cv.notify_all();
        unsafe {
            mapal_par_finish(helped_run);
            mapal_par_finish(blocker_run);
        }
    ...
    extern "C" fn nested_outer(_lo: i64, _hi: i64, frame: *mut u8) {
        ...
        unsafe {
            mapal_par_task(inner, 0, 0, nested_inner, 0, 0, 0, 0, 0);
            mapal_par_launch(inner, frame);
            let entries = [completion_entry(0)];
            mapal_par_wait(inner, entries.as_ptr(), 1);
            nested.inner_trap.store(trap_flag(inner), Ordering::SeqCst);
            clear_trap(inner);
            mapal_par_finish(inner);
        }
        mapal_par_trap(10, 0);
    }
    ...
    fn nested_run_uses_innermost_tls_and_restores_outer() {
        ...
        unsafe {
            mapal_par_task(outer, 0, 0, nested_outer, 0, 0, 0, 0, 0);
            mapal_par_launch(outer, frame_ptr(&*frame));
            let entries = [completion_entry(0)];
            mapal_par_wait(outer, entries.as_ptr(), 1);
            assert_eq!(frame.inner_trap.load(Ordering::SeqCst), (20_u64 << 32) | 2);
            assert_eq!(trap_flag(outer), (10_u64 << 32) | 1);
            clear_trap(outer);
            mapal_par_finish(outer);
        }
    }
    ...
    extern "C" fn dag_task(lo: i64, hi: i64, frame: *mut u8) {
        ...
        if task.is_multiple_of(11) {
            mapal_par_trap(task as i64 + 1, (task % 2) as u32);
        }
    }
            ...
            unsafe {
                ...
                for after in 0..n_tasks {
                    for &before in &frame.predecessors[after] {
                        mapal_par_dep(handle, before as u32, after as u32);
                    }
                }
                mapal_par_launch(handle, frame_ptr(&*frame));

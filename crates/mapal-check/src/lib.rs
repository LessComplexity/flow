//! `mapal-check`: the completion of the pipeline's partial functor
//! (DESIGN `docs/components/check/DESIGN.md`; ADR-0003). Consumes the sealed,
//! `validate()`-clean [`mapal_ir::CategoryIr`] **plus** the
//! [`mapal_syntax::Program`] tree and emits structured diagnostics only,
...
// mapal-lower (only `pub fn lower` is exported; LCode/diag are internal). The
...
/// [`mapal_syntax::Diagnostic`] with `severity: Error`, `fix: None`.
///

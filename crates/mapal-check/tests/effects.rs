...
/// diagnostics.
fn check_src(src: &str) -> Vec<mapal_syntax::Diagnostic> {
    let po = mapal_syntax::parse(src);
    assert!(po.diagnostics.is_empty(), "parse: {:?}", po.diagnostics);
    let ir = mapal_lower::lower(src, &po.program).unwrap_or_else(|ds| panic!("lower: {ds:?}"));
    check(src, &po.program, &ir)
}

fn codes(diags: &[mapal_syntax::Diagnostic]) -> Vec<&str> {
...
/// A pure parallel fanout (`fanout.mapal`: pure `square`/`double` branches) →
/// CLEAN — the pure-fanout baseline.
#[test]
fn pure_fanout_clean() {
    let src = std::fs::read_to_string(format!(
        "{}/../../examples/fanout.mapal",
        env!("CARGO_MANIFEST_DIR")
    ))
    .unwrap();
    let po = mapal_syntax::parse(&src);
    assert!(po.diagnostics.is_empty());
    let ir = mapal_lower::lower(&src, &po.program).unwrap();
...
"#;
    let po = mapal_syntax::parse(src);
    assert!(po.diagnostics.is_empty(), "parse: {:?}", po.diagnostics);
    let err = mapal_lower::lower(src, &po.program)

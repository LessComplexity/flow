...
use mapal_interp::{Outcome, RValue};
...
fn constant_true_loop() -> (CategoryIr, mapal_ir::FuncId) {
...
fn constant_true_loop_diverges_and_returns() {
    ...
    let out = mapal_interp::eval_call(&ir, f, RValue::Scalar(Value::I32(0)), 50);
...
fn constant_true_loop_diverges_at_larger_budget_too() {
    let (ir, f) = constant_true_loop();
    let out = mapal_interp::eval_call(&ir, f, RValue::Scalar(Value::I32(0)), 5_000);
    assert_eq!(out, Outcome::Diverged);
}
...
fn sum_to_n_budget_boundary() {
    ...
    for b in 1..=2000u64 {
        match mapal_interp::eval_call(&ir, f, arg(), b) {
    ...
    assert_eq!(
        mapal_interp::eval_call(&ir, f, arg(), min_done),
        Outcome::Done(RValue::Scalar(Value::I32(55)))
    );
    assert_eq!(
        mapal_interp::eval_call(&ir, f, arg(), min_done - 1),
        Outcome::Diverged
    );
}

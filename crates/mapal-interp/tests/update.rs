...
//!
//! IR is built directly with the `mapal-ir` builder (lower's `c[i] <- x` sugar is
//! WP U3, landing later) — keeps these contracts independent of that crate.

use mapal_interp::{Outcome, RValue, TrapKind, eval_call};
use mapal_ir::{Dest, FuncKind, IrBuilder, SourceLoc, Ty, Value, validate};
...
fn update_u8_program(n: u64, i: u8, v: i32) -> mapal_ir::CategoryIr {
...
fn update_i32_program(n: u64, i: i32, v: i32) -> mapal_ir::CategoryIr {
    let mut b = IrBuilder::new();
    let f = b
        .declare(FuncKind::Named, "main", i32_arr(n), i32_arr(n), L)
        .unwrap();
...
fn f_of(ir: &mapal_ir::CategoryIr) -> mapal_ir::FuncId {
    ir.entry()
}

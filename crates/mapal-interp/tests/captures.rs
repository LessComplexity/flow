...
fn declare_mul_body(b: &mut IrBuilder) -> mapal_ir::FuncId {
    let body = b
        .declare(
            FuncKind::MapBody,
            "mul",
...
fn capturing_map_and_fold_evaluate_through_the_pipeline() {
    ...
    // interp-value half of mapal-lower/tests/captures.rs's product pins —
    // mapal-lower cannot dev-depend on mapal-interp): a capturing map

...
//! STATUS. Mirrors mapal-syntax's bench file style (criterion, `harness = false`).
...
fn build_chain(n: usize) -> mapal_ir::CategoryIr {
        ...
        .unwrap();
    {
        ...
        for i in 0..n {
            ...
            acc = fb
                .binop(mapal_ir::Operation::Add, acc, one, dest, loc)
                .unwrap();
        }
        fb.finish().unwrap();
    }
...
fn build_grid(n: usize) -> mapal_ir::CategoryIr {
    let loc = SourceLoc::new(0, 0);
    ...
    {
        let mut fb = b.build_fn(f).unwrap();
        let mut level: Vec<mapal_ir::ObjectId> = (0..leaves)

//! Golden diagnostics: `tests/fixtures/lex_errors.mapal` exercises every L-code
...
fn lex_errors_all_codes_present() {
    ...
    for t in out
        .tokens
        .iter()
        .filter(|t| t.kind == mapal_syntax::TokenKind::Error)
    {
        let overlaps = out
            .diagnostics
            .iter()

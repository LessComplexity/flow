...
//! P0101..P0116 via `out_of_core.mapal`, and additionally parse the existing
//! `full_surface.mapal` lexer fixture and assert its exact P-code multiset.
...
fn out_of_core_every_p01xx() {
    let src = read_fixture("out_of_core.mapal");
    ...
    for code in [
        ...
        assert!(
            ...
            "expected {code} from out_of_core.mapal; got {counts:?}"
        );
    }
...
fn full_surface_exact_pcode_multiset() {
    ...
    assert!(
        ...
        "full_surface.mapal must lex cleanly; got {:#?}",
        out.diagnostics
            .iter()
    ...
    assert_eq!(
        counts, expected,
        "full_surface.mapal exact P-code multiset mismatch"
    );

    // `List::map -> out;` yields exactly one P0109 and the chain continues: the
    // `over` fn body still has its three statements parsed.
    if let Some(mapal_syntax::Item::Fn(f)) = out.program.items.iter().find(|i| {
        matches!(i, mapal_syntax::Item::Fn(f) if {

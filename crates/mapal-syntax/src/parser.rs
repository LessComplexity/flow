//! The Mapal-Core recursive-descent parser (DESIGN §13–§19).
//!
...
//!
//! The parser is a module of `mapal-syntax`: [`parse`] runs [`crate::lex`]
...
impl<'a> Parser<'a> {
    ...
    fn parse_annotated_item(&mut self) -> Option<Item> {
        ...
        self.diag(
            "P0102",
            at_span,
            "annotations (`@…`) are out of Mapal-Core (HANDOFF §4); planned for Core+1",
        );
    ...
    fn parse_executor_decl(&mut self) -> Item {
        ...
        self.diag(
            "P0111",
            kw_span,
            "`executor` declarations are out of Mapal-Core (HANDOFF §4); planned for post-M5",
        );
    ...
    fn parse_pub_use(&mut self) -> Item {
        ...
        self.diag(
            "P0112",
            kw_span,
            format!("`{what}` is out of Mapal-Core (HANDOFF §4); planned for post-M5"),
        );
    ...
    fn parse_enum_variant(&mut self) {
        ...
        self.diag(
            ...
            "enum/coproduct variants in a `type` body are out of Mapal-Core (HANDOFF §4); \
            ...
            TokenKind::Ident => {
                ...
                if self.at(TokenKind::Lt) {
                    ...
                    self.diag(
                        "P0103",
                        self.span(start, end),
                        "generic type arguments are out of Mapal-Core (HANDOFF §4); \
                         planned for Core+1",
                    );
                    Ty {
    ...
    fn parse_array_type(&mut self) -> Ty {
        ...
        if self.at(TokenKind::Semi) {
            ...
            self.diag(
                "P0104",
                span,
                "dynamic array / slice types (`[T]`) are out of Mapal-Core (HANDOFF §4); \
                 planned for Core+1",
            );
            Ty {
    ...
    fn parse_sigiled_labeled_block(&mut self) -> Stmt {
        ...
        self.diag(
            "P0110",
            sigil_span,
            "labeled blocks (`:label { … }`, ADR-0012) are out of Mapal-Core (HANDOFF §4); \
             Mapal-Core's only loop introducer is `loop`. Planned for Core+1",
        );
        ...
        self.diag(
            ...
            format!(
                "labeled blocks are written `:{ident} {{ … }}` (ADR-0012) and are out of \
                 Mapal-Core (HANDOFF §4); Mapal-Core's only loop introducer is `loop`. \
                 Planned for Core+1"
            ),
        );
        let body = self.parse_block();
    ...
    fn parse_void_stmt(&mut self) -> Stmt {
        ...
        if self.at(TokenKind::LBrace) {
            ...
            self.diag(
                "P0113",
                kw,
                "`void` is out of Mapal-Core (HANDOFF §4); planned for Core+1",
            );
    ...
    fn parse_bind_stmt(&mut self) -> Stmt {
        ...
        let index = if self.at(TokenKind::LBracket) {
            ...
            while self.at(TokenKind::LBracket) {
                ...
                self.diag(
                    ...
                    "nested element-update target `c[i][j] <- x` is out of Mapal-Core \
        ...
        if self.at(TokenKind::BackArrow) {
            ...
            self.diag(
                "P0008",
                span,
                "only one `<-` binding per statement (W17); chained `<-` is not Mapal-Core",
            );
        ...
        match self.kind() {
            ...
            TokenKind::Colon if self.peek_kind(1) == TokenKind::Ident => {
                ...
                self.diag(
                    "P0110",
                    span,
                    "labeled jumps (`-> :label`, ADR-0012) are out of Mapal-Core (HANDOFF §4); \
                     Mapal-Core's only back-edge jump is `-> loop`. Planned for Core+1",
                );
    ...
    fn parse_ident_stage(&mut self) -> StageKind {
        ...
        if self.peek_kind(1) == TokenKind::LBrace && self.ident_brace_is_op_block() {
            ...
            self.diag(
                ...
                name_span,
                format!(
                    "`{op}` is out of Mapal-Core (HANDOFF §4): only `map`/`fold` are Core \
            ...
            match self.kind() {
                ...
                TokenKind::LParen => {
                    ...
                    self.diag(
                        "P0116",
                        span,
                        "destructuring op-block parameters (`(x, y)`) are out of Mapal-Core \
    ...
    fn parse_classified_block(&mut self) -> StageKind {
        ...
        if all_headless {
            ...
            self.diag(
                "P0115",
                span,
                "anonymous block stage (`-> { … }` with statements/tail) is out of Mapal-Core \
    ...
    fn parse_guard_discr(&mut self) -> (GuardDiscr, SourceLoc) {
        ...
        if self.at(TokenKind::Minus) && self.is_pattern_arm() {
            ...
            self.diag(
                "P0106",
                span,
                "pattern guard arms (`-Some(x)->`, `-[h, ...t]->`) are out of Mapal-Core \
        ...
        loop {
            match self.kind() {
                ...
                TokenKind::LParen => {
                    ...
                    // ADR-0031: no call expressions in Core, uniformly — the
                    // ADR-0029 iota/fill carve is superseded; they are pipeline
                    // stages (`n -> iota`, `(x, n) -> fill`).
                    ...
                    self.diag(
                        ...
                        span,
                        if is_array_builtin {
                            "call expressions are out of Mapal-Core; `iota`/`fill` are \
                             pipeline stages — write `n -> iota` / `(x, n) -> fill` (ADR-0031)"
                        } else {
                            "call expressions are out of Mapal-Core (HANDOFF §4); \
                ...
                TokenKind::Question => {
                    ...
                    self.diag(
                        "P0101",
                        q,
                        "the `?` operator is out of Mapal-Core (HANDOFF §4); planned for Core+1 \
                         error handling",
                    );
                ...
                TokenKind::Colon if self.peek_kind(1) == TokenKind::Colon => {
                    ...
                    self.diag(
                        ...
                        "`::` paths are out of Mapal-Core (HANDOFF §4); planned for Core+1 modules",
                    );
    ...
    fn parse_array_literal(&mut self) -> Expr {
        ...
        if self.at(TokenKind::RBracket) {
            ...
            self.diag(
                ...
                "empty array literal `[]` has no element type and is not Mapal-Core surface",
            );
            return Expr {
        ...
        while !self.at(TokenKind::RBracket) && !self.at_eof() {
            ...
            if self.at(TokenKind::DotDotDot) {
                ...
                self.diag(
                    ...
                    "rest elements (`...tail`) in array literals are out of Mapal-Core \

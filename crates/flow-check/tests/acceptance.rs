...
use flow_ir::{CategoryIr, FuncKind, IrBuilder, SourceLoc as IrLoc, Ty, Value};
use flow_syntax::Program;
...
/// Parse + lower a Flow source into `(program, sealed IR)` (asserting parse +
...
fn cross_pass_order_is_exclusivity_then_effects() {
    ...
    let po = flow_syntax::parse(TWO_VIOLATIONS_SRC);

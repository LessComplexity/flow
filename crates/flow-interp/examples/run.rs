//! Runnable dev tool: run a `.flow` file through the interpreter (the oracle).
//!
//! ```text
//! cargo run -p flow-interp --example run -- path/to/file.flow
//! cargo run -p flow-interp --example run -- examples/fir.flow --budget 1000000
//! ```
//!
//! Runs the real pipeline (`flow_syntax::parse` → `flow_lower::lower` →
//! `flow_interp::run`) and prints the program's output to stdout. The run is
...
//!
//! This is the `flow run` slice, available before `flow-cli` exists — it is a dev
//! example, not the user-facing CLI (flow-cli owns real diagnostic rendering, C3).

use std::process::ExitCode;

use flow_interp::{Outcome, RunResult, TrapKind, run};
...
fn main() -> ExitCode {
    ...
    let Some(path) = args.iter().find(|a| !a.starts_with('-')).cloned() else {
        eprintln!("usage: cargo run -p flow-interp --example run -- <file.flow> [--budget N]");
    ...
    let ir = match flow_lower::lower(&source, &parsed.program) {
        Ok(ir) => ir,
        Err(diags) => {
            report(&path, &lines, "lower", &diags);
            eprintln!(
                "run: {} lower error(s) — rejected (out of Flow-Core, or ill-typed).",
                diags.len()
            );
            return ExitCode::FAILURE;
        }

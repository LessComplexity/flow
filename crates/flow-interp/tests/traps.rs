...
use flow_interp::{Outcome, RValue, TrapKind, run};
...
fn div_program(a: i32, b: i32) -> flow_ir::CategoryIr {
...
fn index_program(i: i32) -> flow_ir::CategoryIr {
    let mut ib = IrBuilder::new();
    let main = ib
        .declare(FuncKind::Named, "main", Ty::IoToken, Ty::IoToken, loc())
        .unwrap();
...
fn cmp_nan_program(op: Operation) -> flow_ir::CategoryIr {
    let mut ib = IrBuilder::new();
    let main = ib
        .declare(FuncKind::Named, "main", Ty::IoToken, Ty::IoToken, loc())
        .unwrap();

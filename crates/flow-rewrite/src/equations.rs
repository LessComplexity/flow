...
use flow_ir::{CategoryIr, MorphismId, ObjectId, ObjectKind, Operation, Value, ty_contains_token};
...
fn constify_zero(ir: &CategoryIr, plan: &mut RewritePlan, o: ObjectId) {
    let v = match &ir.object(o).expect("obj").ty {
        t if *t == flow_ir::Ty::i32() => Value::I32(0),
        t if *t == flow_ir::Ty::i64() => Value::I64(0),
        t if *t == flow_ir::Ty::u8() => Value::U8(0),
        _ => return,
    };
    plan.constify.insert(o, v);
}

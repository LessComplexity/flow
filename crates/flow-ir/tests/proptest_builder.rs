...
use flow_ir::{Dest, FuncKind, IrBuilder, Operation, SourceLoc, Ty, Value, lint_mermaid, validate};
...
proptest! {
    ...
    fn interleaved_calls_seal_implies_valid(steps in prop::collection::vec(step_strategy(), 0..40)) {
        ...
        let mut pool: Vec<flow_ir::ObjectId>;
        {
            ...
            let mut loops: Vec<flow_ir::LoopHandle> = Vec::new();
            let pick = |pool: &Vec<flow_ir::ObjectId>, i: usize| pool[i % pool.len().max(1)];
            for step in steps {
                if pool.is_empty() {
                    break;
                }
                let res: Result<flow_ir::ObjectId, _> = match step {
...
fn determinism_same_program_same_output() {
    fn build() -> (
        String,
        Vec<flow_ir::MorphismId>,
        Vec<Vec<flow_ir::ObjectId>>,
    ) {
        let mut b = IrBuilder::new();
        let f = b

...
-- way to get *Flow's* logo the same way is to put it in a font — which
-- assets/font/FlowIcons.ttf does, one glyph at U+F8F0.
--
-- Registration is automatic (see plugin/flow_icon.lua); calling setup() by hand is
-- optional and only needed to override the glyph or colour.
--
--     :FlowIcon                       -- report what is actually registered
--     require("flow.icon").setup({ glyph = require("flow.icon").logo })

local M = {}

-- The real Flow mark, from assets/font/FlowIcons.ttf. Renders as a blank box unless
...
M.name = "Flow"
M.hl = "FlowIcon" -- our own group, so the icon is teal rather than a borrowed colour
...
function M.setup(opts)
  ...
  vim.api.nvim_create_autocmd("ColorScheme", {
    group = vim.api.nvim_create_augroup("FlowIconHl", { clear = true }),
  ...
  if ok_mini then
    ...
    cfg.extension.flow = { glyph = glyph, hl = M.hl }
    cfg.filetype = cfg.filetype or {}
    cfg.filetype.flow = { glyph = glyph, hl = M.hl }
...
--- Is FlowIcons.ttf actually installed? Best-effort, per platform.
local function font_installed()
  local candidates = {
    vim.fn.expand("~/Library/Fonts/FlowIcons.ttf"),
    vim.fn.expand("~/.local/share/fonts/FlowIcons.ttf"),
    vim.fn.expand("~/.fonts/FlowIcons.ttf"),
  }
  for _, p in ipairs(candidates) do
    if vim.fn.filereadable(p) == 1 then return true, p end
  end
  if vim.fn.executable("fc-list") == 1 then
    if vim.fn.system("fc-list | grep -c FlowIcons"):gsub("%s", "") ~= "0" then
...
function M.report()
  ...
  table.insert(lines, ("  FlowIcons.ttf   : %s"):format(
  ...
  if g == M.logo and not have then
    ...
    table.insert(lines, "    install assets/font/FlowIcons.ttf, or drop the glyph override.")
  elseif not have then
    table.insert(lines, "  note: install assets/font/FlowIcons.ttf and set")

" Vim syntax file
" Language:    Mapal (dataflow language — mapal-lang Mapal-Core subset, v0.3)
" Maintainer:  Mapal project (editors/nvim)
" Reference:   docs/spec/user-guide.md §3 (as patched: E4 statement rule,
"              E5 `type` keyword, LC-3 sigiled labels), HANDOFF.md §4.1
"              (Mapal-Core scope), ADR-0012 (labeled blocks `:label { … }`,
"              jumps `-> :label;`).
"
" Regex-based highlighting. A tree-sitter grammar is deferred until it can be
" derived from the real mapal-syntax parser (ADR-0008).
"
" ----------------------------------------------------------------------------
...
" Case matters in Mapal (snake_case values, PascalCase types).
syn case match
...
"   fn type loop seq mut void  — all real Mapal-Core keywords.
...
" ---------------------------------------------------------------------------
" Mapal-Core primitives (HANDOFF §4.1).

...
| **O4** | Provable cross-target correctness layer | *None portable*; CompCert (CPU-only certified); MLIR/XLA/TVM are heuristic (bug-for-bug target drift) | "Provable" only as strong as the functor proofs (the E1 trace theorem is still informal); correctness alone doesn't sell if performance is poor; market may not *pay* for correctness | A claim **no incumbent makes**; decisive in safety-critical/regulated domains and silicon validation; defensible moat; already the spec's core thesis (§8) |
...
2. **The field is crowded with heavy hitters pursuing this exact thesis** — MLIR,
   ...
   *validates* the thesis but means "be the universal layer" is a fight on
   everyone's turf. Flow needs a wedge they lack.

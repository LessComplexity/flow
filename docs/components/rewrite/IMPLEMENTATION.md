...
| --- | --- | --- | --- |
| `analyze_const_fold` | `&CategoryIr → RewritePlan` (layer 3: oracle-exact folds, integer/bool identities, proj∘pack, index-of-const, phi-select; P1/P2 + token guard in `forward`) | `src/equations.rs:analyze_const_fold` | built |
...
| `replay` | `(&CategoryIr, &RewritePlan) ⇀ CategoryIr` (recipe classification §1.1; loop quartet with per-merge-SCC exit attribution (S12); fused-body synthesis via verbatim inline; fn-level DCE via post-plan reference liveness) | `src/replay.rs:replay` (+ `is_canonical`, `synthesize_fused`, `inline_body`) | built |
...
| Item | Realised at |
| --- | --- |
| testgen (random Core programs over the public builder; closed+open, default+trap_free — feeds P5–P7) | `tests/testgen/mod.rs` |

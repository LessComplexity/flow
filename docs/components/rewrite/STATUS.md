...
Status: tested
Last updated: 2026-07-28 · S40 (plan-s40-the-arm-owns-the-loop, ir-led; this crate's share) — **three rewrite-side defects from S40's adversarial review, fixed and pinned** (`tests/guard_loops.rs`, 6 tests; full account plan-s40 §6b): (1) **DCE's dead-Phi pin keys on `gated()`, not `can_trap`** — a heavy-but-trap-free gated site (arm owning a non-terminating trap-free loop) lost its Phi while RW11 kept the loop: Done raw, Diverged rewritten. (2) **ConstFold drops a resolved guard's losing arm only when the phi-select alias provably fires** (mirrors `forward`'s SCC/token refusal) — dropping while the Phi survived made replay rebuild a boundary edge against a dropped feeder ("feeder is not mapped" panic); and a Phi whose arm owns a `LoopEnter` handle is not folded at all (dropping a loop unit is replay surgery; the fixpoint refolds after `LiftLoops`). (3) **DCE preserves dead cones that can touch a guard verdict** — gating is consumer closure, the interpreter executes dead code, so dropping a dead sibling reader of a trap-capable value flipped a site strict→gated and suppressed a trap (`Trapped(DivZero)` raw → `Done` rewritten; pre-existing S39-class, reproduced independently by the 1024-case hammer). The pin is the dead `Temporary` SINK (replay materializes only read objects — pinning interior products does not survive the rebuild), restricted to sinks forward-reachable from the verdict cone AND tainted by an observable-when-executed morphism, so pure dead cones still drop and surface emissions are unmoved (A/B: 103/104 byte-identical, `calc` only). Two rejected fix paths recorded in plan-s40 §6b, incl. why dead-sink *ownership* in `guard_arm` is unsound (cross-arm dead cones). `Step::PhiTrapArm` + loop steps give the class differential coverage; three LiftLoops seeds + one DCE seed pinned. Previous: 2026-07-26 · S34 — **the P0 is fixed: `map(id) → id` deleted traps.** `is_identity_body` judged a body by its Return writer alone, so a body that returns its parameter *and* computes a dead trapping `Div` (the shape ConstFold's `proj∘pack` forwarding produces) read as the identity and the whole `Map` was aliased away — `Trapped(DivZero)` became `Done(0)`. The guard now quantifies over the body's entire morphism set through the crate's one purity predicate (`graph_rewrites::is_pure`, now `pub(crate)` — DCE's "may this dead cone go?" and the identity law's "does this body denote `id`?" are the same question). All four failing property entry points were this one bug; the pinned proptest seed is **retained** and now passes. 70 in-crate tests (+2: the hand-written adversarial case and its positive control, both negative-controlled). Detail: `plans/plan-s34-identity-map-trap.md`. Previous: 2026-07-25 · S29 — `replay.rs:emit_op` gained a `TimeMs` arm so a clock-bearing program can be replayed at all (`fb.time_ms` off the bare token). **No pass plans it**: the existing purity/token predicates exclude it from CSE, forwarding, DCE and lifting with no `TimeMs` special case, exactly as intended for an effectful op. 68 in-crate tests, unchanged — nothing here generates a clock read yet (see known issues). Previous: 2026-07-24 · S27b **loop→Map/Fold lifting SHIPPED** (`plan-loop-to-map-fold.md`, ratified option 2): `PassId::LiftLoops` runs immediately after default-first Inline. `analyze_lift` consumes `mapal_ir::loop_plan` and records `LiftSpec` by merge; replay replaces the SCC with a minted count + `Iota(K)` + synthesized captured Map/Fold body. R-LF and R-LM require `K >= 1`; zero-trip loops stay loops. Every rejection is pinned. The matmul4 chain now completes across fixpoint rounds: cell fold-lifts → Inline strips it → outer loop map-lifts → `tile_plan` fires; the final graph has 0 Calls, 0 loop SCCs, and Map-with-Fold, with exact `-275\n3748\n` LLVM output at O0/O2 and default/`MAPAL_PAR=1`. Testgen supplies both lift shapes to the 1,280-run differential; 68 in-crate tests green. Previous S27: fn-strip wired default-first, loop-bearing-callee guard, cap 256, body Call stripping.

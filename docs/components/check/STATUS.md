...
- `check(source, program, ir) -> Vec<Diagnostic>` — the two owed passes, **25 tests green**:
  - **T0101 Return exclusivity** (DESIGN §3): >1 full-value writer per Return rejected (strict CK3); discharges interp IN3 / ir §17.
  - **T0201 E2 effect legality** (DESIGN §4): effectful morphism inside a `FanoutKind::Plain` branch rejected, message names `seq` (ADR-0003 mandate); `FanoutKind`-keyed sticky context (inner `seq` does not legalize, CK5); scope-aware call resolution; `effectful?` deduced from lowered token signatures.
...
- Nothing red. E2 nested-`seq` reading (CK5) is a conservative pin — OQ-C1 flagged for Sapir, revisable by one-line ADR.
...
- 25 tests: 11 acceptance (9 examples, determinism, cross-pass order) · 6 exclusivity (hand-built IrBuilder multi-writer shapes, slot form, loops incl. token-bearing) · 8 effects (all DESIGN §7.3 cases incl. the FanoutKind::Seq same-node-kind trap and T0201 span pins).
...
- OQ-C1: nested-`seq`-in-fanout legality (CK5 conservative pin) — Sapir, one-line ADR to loosen.

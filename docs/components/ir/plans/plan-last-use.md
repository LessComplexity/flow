...
All three are one analysis, and it belongs in flow-ir next to `loop_plan` — computed once, every backend inherits it, property-testable in one place.
...
- **Measured (box sweep):** loop-form matmul wall + FLOW_PERF launch counts/kinds; llvm loop-form already at the 0.01 s floor locally — the sweep keeps it there (regression watch).

## 6. Sequencing

1. `flow-ir`: the query + property tests (determinism, totality, agreement with a brute-force reference over testgen graphs).

...
Status: tested
Last updated: 2026-07-18 · Session 13 (ADR-0021 `Update` op + `loop_plan`/`LoopPlan` — the one-source-of-truth loop CFG, BL7)
...
- 30-variant Core `Operation` set (ADR-0013 + ADR-0018 + ADR-0021): per-slot `Pair{slot,arity}` product formation, `Proj`, arith/cmp/logic + `Neg`, `Phi`, `Call`, `Map`/`Fold` (bodies as non-first-class FuncDefs), `Index`, `Update` (array element write `(Array,I,T)→Array`, OOB traps like `Index`; ADR-0021), `Zip`/`Enumerate` (collection primitives, ADR-0018), `Print`, `LoopEnter`/`LoopBack`/`LoopExit`, `Output`.

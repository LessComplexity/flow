...
- 26-variant Core `Operation` set (ADR-0013): per-slot `Pair{slot,arity}` product formation, `Proj`, arith/cmp/logic + `Neg`, `Phi`, `Call`, `Map`/`Fold` (bodies as non-first-class FuncDefs), `Index`, `Print`, `LoopEnter`/`LoopBack`/`LoopExit`, `Output`.
...
- Iterative Tarjan `sccs(f)`, Kahn `topo_order(f)` (LoopBack emitted-not-gating, header-first), `loop_structure(f)` — the backend-verilog capability predicate (single-loop accept vs multi-merge reject shapes tested).

...
|---|---|---|
| `topo_order` | `CategoryIr × FuncId → MorphismId*` | the dataflow/execution order (Kahn, `LoopBack` non-gating; **ties on source position — S38, §13**). A *different* ordering of the same edge set than the stored insertion (construction) order — a pure function of adjacency **and `loc`**, so storing it would be a copy that could drift. Since S38 `SourceLoc` is therefore a **semantic** attribute, not debug metadata: every rewrite owes it a discipline exactly as it owes value-preservation (open, plan-s38 §6.2 — ADR candidate) |
...
| `path_plan` | `CategoryIr × FuncId → PathPlan` | the execution graph's task DAG + host-spine checkpoints (S24, plan-parallel-orchestrator). Partition, deps, ranks and waits are pure functions of adjacency + `topo_order` + `loc`; storing them would be a copy that drifts from the graph. Backend-independent — one plan, every runtime schedules it. The clock rules (S29's fence on **source** position and host cone, S36's read-as-a-task) live here — see §13 |
| `guard_plan` | `CategoryIr × FuncId → GuardSite*` | per Phi-position guard: the condition object and each arm's **exclusive work** — the morphisms that run only when the condition picks that arm (S39, plan-s39). Ownership is CONSUMER CLOSURE: a morphism is arm-owned iff every consumer of its target is arm-owned. Liveness is NOT the test — nothing deletes dead code before execution, so a dead consumer still reads its operand. Loops join as UNITS through their `LoopEnter` handle (S40, plan-s40) — machinery is never per-morphism ownable, and the handle carries the unit's flags. `can_trap`/`heavy` are transitive through nested sites; `GuardSite::gated()` is the legality-then-cost predicate consumers filter on |
...
- `topo_order(f: FuncId) -> Vec<MorphismId>` — Kahn's algorithm over the function's morphisms with **LoopBack edges excluded**, where a morphism becomes ready when its source object is *complete*: Parameters/Constants start complete; a product object completes when all `arity` slot edges have been emitted; a LoopMerge completes on its `LoopEnter` alone (header-first canonical order, §5.2); other objects complete on their one definer. **LoopExit edges are ordinary gating edges** (their source route must complete first; their target — outside the SCC — is ordered after the loop body). Only LoopBack is special: appended after its source's completion, never gating (it is *in* the output — interp needs it). **Ties broken by SOURCE POSITION — `(loc.start, loc.end, insertion index)` (S38, plan-s38-trap-order-is-source-order).** The ready worklist is a min-heap on that key, not a FIFO. *Insertion order is a property of the compiler; trap order is observable, so it must be a property of the program.* The dataflow graph does not order two independent trapping operations at all, and rewriting creates and destroys objects — so an insertion-order tie-break let `Inline` turn `Trapped(IndexOob)` into `Trapped(DivZero)`, breaking `eval ∘ rewrite = eval`. Source position is the only intrinsic key available. This is S29's clock-read fence generalised: there too, "the graph orders pure work against a clock read not at all" and source order was the tie-break with meaning. The insertion index keeps the key total, so the walk stays deterministic when a desugaring emits several morphisms from one span; exact `loc` ties (a Parameter and Return sharing the function span, loop objects sharing the loop span) therefore fall back to insertion order, and none of those objects trap on their own. Multi-merge SCCs (nested loops, §7) order their headers by the same key. **Consequence, deliberate and measured:** this order also drives emission, so it is observable beyond trap kind — a trap can no longer pre-empt output written before it in the source (pinned by `backends/llvm/tests/differential.rs::differential_trap_preserves_preceding_output`, a verified negative control), and every backend's emission order moved (38 goldens, adjudicated ordering-only except that one intended behaviour change). **LoopEnter deferral (S12):** a ready `LoopEnter` is released only when no other morphism is ready — so every morphism not transitively gated by a merge (every *loop-invariant* computation, however many hops from its sources) precedes the loop header. Consumers rely on this: the interp driver reads invariant operands when the header fires, and straight-line backends emit invariants before the loop. Before S12, FIFO readiness ordered *derived* invariants (`x * 2` — pair then Mul) after `LoopEnter`, and the driver read them before write (found by a user matmul program; pinned by `topo_orders_multi_hop_invariants_before_loop_enter` and interp `loop_invariants.rs`).
...
- `guard_plan(f: FuncId) -> Vec<GuardSite>` — the **gated arms of every Phi-position guard** (S39,
  plan-s39). A guard is a gate on the flow: an arm's exclusive work runs only if the condition selects
  that arm, so `(1 > 0) -> { -true-> 42; -false-> 7 / 0; }` returns 42 instead of trapping. Arms are
  still *compiled*; only *running* is conditional. Each `GuardArm` carries `own` (its exclusive
  morphisms, topo-ordered, boundary `Pair` edge last), `can_trap` and `heavy`. Three rules earned by
  defects rather than derived: **(1)** ownership is consumer closure, not liveness — a `Proj` feeding
  both an arm and a *dead* `Neg` is not arm-owned, because the interpreter evaluates the dead `Neg`
  and would read an object the unchosen arm never wrote; **(2)** stripping a nested site's work from
  its enclosing arm can orphan a morphism, so closure is re-established to a fixpoint afterwards;
  **(3)** `can_trap`/`heavy` are **transitive** through nested sites — `calc`'s right-folded 5-arm
  match hides its `Div`/`Mod` in the innermost site, and without transitivity the outer arms went
  ungated and `calc(0, 20, 0)` still trapped. `gated()` is legality-then-cost: an arm that can trap
  must be gated (no cost argument buys it), a `heavy` arm (bulk op, call or loop) is worth gating, and
  two arms of scalar arithmetic are left alone — branching around `x * -1` costs more than computing
  it, and inside a map body it would cost the loop its vectorization. **(4, S40 — plan-s40:)**
  **loops join an arm as UNITS.** A loop's unit is the whole region the flat walk hands the driver
  (SCC-incident morphisms + machinery + the `loop_plan` cones). Machinery is never per-morphism
  ownable — per-morphism closure provably cannot complete a cycle, so the only machinery it can
  reach alone is the exit boundary, and gating that fragment starves the driver. A unit joins iff
  every consumer outside it of every member's target is already owned, and it is represented in
  `own` by its `LoopEnter` handle(s) alone: internals stay out, so subtraction, re-close and the
  consumers' skip-sets never see them — the driver fires them, invoked from the Phi. The handle
  carries the unit's transitive `can_trap`, and a loop is `heavy` by definition. A non-canonical
  unit (no `loop_plan`) never joins — it runs unconditionally, which is always safe, and `LiftLoops`
  consumes `loop_plan` too, so an undrivable shape is also unliftable: no instability. A site whose
  boundary edge is itself a unit member never joins that unit (from inside, every "external"
  consumer is vacuously satisfied — the driver already runs the site). This replaced the v1
  refusal, which keyed the site's *semantics* on graph shape; `LiftLoops` removes the SCC, so the
  same site was strict raw and gated rewritten — `eval ∘ rewrite = eval` broken (S39 §4a, three
  pinned seeds).
...
- `path_plan(f: FuncId) -> PathPlan` — the **execution graph's task DAG** (S24; plan-parallel-orchestrator, ratified backend-independent). Pure work is partitioned into `Task { kind: Split{site,n} | Seq{morphisms}, deps, rank, trap_min, pinned }` in first-topo-occurrence order (so task ids are deterministic), and the host spine gets a `Checkpoint { topo, wait: [WaitEntry{task, threshold}] }` at every token operation and at function exit (`threshold: Some(w)` = a decided watermark ≥ `w` suffices, `None` = the task must have *completed*). Token-bearing morphisms and every morphism of an effectful loop region stay on the spine. Two rules are keyed on the clock read (S29, plan-time-builtin composition rules 4 and 5) — both are *scheduling* facts the dataflow graph alone cannot supply:
  - **The fence, and the read is a task (S36, plan-s33b).** A clock read is its own **pinned** `Seq` task, and its fence is that task's `deps`: every task all of whose morphisms begin before the read **in the source** (`Morphism.loc.start` vs the read's). Source position, not topo position, because the graph orders pure work against a clock read *not at all*: a `TimeMs` consumes only the token, has no value producer to wait for, and topo order therefore legally (and in practice) schedules the bracketed work after the closing read. Source order is exactly what a programmer means by putting two reads around some lines, and it is what makes `t1 - t0` measure the work *written* between them — including the S28 case of opening the bracket after data generation to exclude it. **The mirror half is the point of making it a task:** every task all of whose morphisms begin *after* the read gains a dependency **on** it, so the bracketed work cannot be dispatched until the read has happened. Without it the read has `before` and no `after` — the DAG unlocks the kernel the moment its inputs are ready, while the host is still walking toward the clock, and `t1 - t0` measures an interval the work already left (measured at 6/100 threaded fir 65 536 runs reading under 0.01 ms for a ~0.07 ms kernel). The two sets are disjoint (`max_loc < start` vs `min_loc > start`) and a read is in neither of its own, so no clock edge can close a cycle. A clock read that stays on the spine — inside an effectful loop region, where `host_loop_member` claims it — keeps its `Checkpoint` instead, which is the only fence it has.
  - **The host cone.** `TimeMs` is the first spine op producing a **value** rather than only a token, and pooled tasks are dispatched before the host writes that value; a *pooled* task consuming a clock read therefore races the write — FRAMEWORK §4.5 **Law 1** (placement honesty: no transformation reads data not present at its location), a data teleport, whose observed symptom was a NEGATIVE elapsed. The whole consumer cone of a clock read (a single forward sweep in topo order) stays on the spine. In practice that cone is scalar arithmetic and costs nothing; a bulk op fed by a clock read is pinned sequential — correct before it is fast. **S36 narrows this to the cone, not the read:** the read itself is a task, but a *pinned* one, so the host spine still executes it at its own topo position and writes the frame slot there — the consumers reading that slot are the ones that must not run on a worker.

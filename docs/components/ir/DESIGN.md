...
| I9 | Core types only: scalar whitelist of §3; `Tuple` arity ≥ 2; `Array` size ≥ 1; `Str` restricted (next row). Intake = **every** Ty entering the builder, *including synthesized tys* (`pack`→Tuple, `pack_array`→Array, `binop`/`phi`/`index`/`call`/`map`/`fold`/`print`/route packs) — not just caller-supplied annotations | builder at every Ty intake, declared or synthesized |

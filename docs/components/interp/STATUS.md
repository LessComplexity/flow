...
Status: tested
Last updated: 2026-07-18 · Session 12 (two loop-driver P0 fixes — computed invariants via ir topo LoopEnter deferral; per-merge exit attribution)
...
- Out-of-M1 loop shapes (multi-merge SCC, >1 `LoopBack`, ≠1 attributed `LoopExit` **per merge's own SCC**) are surfaced via `assert!` in `derive_plan`, not a returned error variant (`Outcome` has no error case; DESIGN §9 classes these as `unreachable!`-class; the multi-merge nested shape IS lower-reachable — rewrite RW8/F3 — but never produced by the examples/testgen). **S12:** attribution is per-merge-SCC, never the per-fn union — the union falsely tripped this assert on two sequential canonical loops (legal Core, now supported + pinned); and multi-hop computed loop-invariants (`x*2`, `a[i*4+k]`) previously read-before-write-panicked the driver — fixed by ir §13's LoopEnter deferral (both pinned in `tests/loop_invariants.rs`).

...
| loop-layout derivation (§4) | `merge → LoopPlan` (decide/advance split) | `crates/flow-interp/src/loops.rs:derive_plan` | built |
...
| The 55 contract / D7 — exit reads the exit-iteration merge state (guard-first, ADR-0016) | `crates/flow-interp/src/loops.rs:run_loop` (decide→guard→advance) · `derive_plan` (decide/advance split) | `tests/acceptance.rs::sum_to_n_value_contract` · `tests/acceptance.rs::golden_sum_to_n` |
| Guard-first (ADR-0016) — advance set not evaluated on the exit step | `crates/flow-interp/src/loops.rs:derive_plan` (`decide_order`/`advance_order`) · `run_loop` | `tests/acceptance.rs::golden_fir` · `tests/acceptance.rs::golden_countdown` |
...
- **`LoopPlan` is a derived record with no named model object.** DESIGN §4 says the driver
  "derives them once per merge" but names no object; `crates/flow-interp/src/loops.rs:LoopPlan`
  is that memo. **Consistent** — it is the deduced §4 layout, not new state.
- **In-SCC representation diverges from DESIGN §2.** §2 prescribes folding `loop_structure`
  ...
  membership-only (`SecondaryMap<ObjectId, ()>` in `build_in_scc`) and re-derives the merge in
  `derive_plan`. Behaviourally equivalent for M1's single-merge SCCs; the object→merge fold is

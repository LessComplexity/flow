...
3. **otherwise** (not incident, `op ∉ {LoopBack, LoopExit}`) → `eval_morphism(m)` (§3): read `env[m.source]`, apply `m.op`, write `env[m.target]`. Decrement `budget`; `0 ⇒ Err(Diverged)`. This computes the loop-invariant inputs (before the driver fires) and the post-loop consumers (after it has written the exit objects).

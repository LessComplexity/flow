...
Scope: one new deduced query in `flow-ir` (backend-independent), a small scheduler runtime in `flow-rt`, and `backend-llvm` emission that consumes both. Zero language/IR-op/oracle change. cuda/verilog consume the same query in their own later waves (§3) — nothing there changes now.

## 0. The main idea (the invariant this plan must not diverge from)

Flow is parallel-first: the execution graph separates into **execution paths**; each path is its own concurrent unit; the backend maps paths onto whatever its platform runs concurrently — CUDA blocks/streams, FPGA/ASIC regions, CPU threads. The graph's divergence is known at **compile time**, so each backend gets a statically-known schedule and builds the best runtime for it — for a CPU with 12 logical cores: 12 threads, each executing a path, the next path waiting in line.
...
3. **Trap protocol — speculate-and-order (v3, revised during implementation review; supersedes the abandon-and-skip draft).** Task-context trap guards never call `flow_trap`: a failing guard **records** `(site topo, kind)` into `TrapFlag` (CAS-min by topo) and **continues with a zero dummy** — task execution is total and defined everywhere (no abandon, no skip propagation, no partial-task corners). *Soundness:* every morphism topo-before the true first trap `t*` computes on real values (dataflow sources precede consumers in topo), so the minimum recorded topo IS `t*`; garbage-induced records are always > `t*` and lose the CAS-min. Each task publishes a **decided-watermark** (release atomic): "all my trap sites ≤ W have executed their guards" — scalar tasks bump it per site; bulk/loop tasks publish only at completion (their sites topo-follow all their inputs by the LoopEnter-deferral theorem, so no per-iteration store — the GEMM inner fold pays nothing). A `Checkpoint` waits until every guard task's watermark passes its topo (or the task completed) and its consumed values' producer tasks completed, **polling the flag each spin**: `flag.topo <` checkpoint ⟹ the HOST calls `flow_trap(kind)` (exit 101) — else it prints. Exit checkpoint: all tasks + final flag check. Host-spine sites (token chain, effectful loops, pinned tasks) keep today's direct `flow_trap`. Workers never exit, park, or unwind; a garbage-fed diverging loop can only stall a point whose oracle run also diverges (same theorem). Still the cuda shape — record on the worker, observe at the spine's sequential point — with the observation rule made order-exact.
...
**Consolidation (§3).** No parallel twin of anything: the sequential run is the same plan executed by a pool of one (or `FLOW_PAR=1`) — one emission form, one runtime, degenerate placement. And the query is ONE object consumed by all backends — not a per-backend re-derivation.
...
## 4. The CPU runtime (`flow-rt` — the scheduler)

Small static-DAG executor, dependency-free (std only, threads ride pthreads):

- **Pool:** T = `FLOW_PAR` if set else `available_parallelism()` (12 logical cores ⟹ 12 threads), spawned once per process, parked when idle.
...
- **Static tables:** the emitted program hands flow-rt, per fn invocation, a compile-time table: task fn pointers, kinds, ranks, dep counts, dependent lists, guard sets — the "best runtime built from compile-time knowledge": the scheduler discovers nothing, it executes a known DAG.
...
- The host fn becomes: materialize frame → register the static task table (`flow_par_begin/task/pin/dep/launch`) → host walk with `flow_par_wait`+`flow_par_check` at each checkpoint's injection point → `flow_par_finish` before the epilogue. Two injection refinements (both review finds, S24): a checkpoint's wait fires at the earliest host glue morphism that reads task-produced data (not at the token op itself), and a checkpoint living inside an effectful loop ALSO fires once before the loop's first `LoopEnter` — the loop's seed reads are otherwise unordered against the tasks.
...
| Where the query lives | `flow-ir` (backend-independent) | every backend maps the same paths to its own layout (§3) |
...
| `PAR_GRAIN` | 4096 elements (flow-rt const) | below it slice overhead rivals work; *measured knob, bench wave revisits* |
| Thread count | `FLOW_PAR` env else all logical cores | one knob; `=1` is the sequential A/B lever |
...
1. **Graph analysis** (`flow-ir`): a function that reads a compiled program's graph and answers "which parts are independent, which waits for which, where are the prints, which parts can trap." Pure analysis + its own unit tests; changes no behavior anywhere.
2. **Scheduler** (`flow-rt`): the thread pool + queue described in §4 — ~200 lines, no dependencies. Unit tests: every index covered exactly once, dependency order respected, trap flag picks the oracle's trap.
...
- The scheduler is the riskiest piece (nested calls + help-first). Mitigation: the plan degenerates to sequential under `FLOW_PAR=1` on every differential case — bugs surface as A/B diffs, not mysteries.

## 10. Open questions

- None blocking beyond the gate. (cuda-streams consumption of `path_plan` and the ADR-0028 tree-fold both stay queued as their own waves; `FLOW_PAR`/`PAR_GRAIN` stay harness knobs, not language surface — same class as the deferred `time` builtin.)

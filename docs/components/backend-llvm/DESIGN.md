...
| `TimeMs` **(S29, plan-time-builtin)** | `call double @mapal_time_ms()` + store. The source `IoToken` erases and the `(IoToken, f64)` target's residual has arity 1, so it materializes as the **bare** `double` (the §2 erased-representation rule) — the call result IS the target's value, no pair is built. Emission position in the block IS the ordering the token models (L4). The op is effectful: it is never rewritten, and a fn containing it stays attribute-free through the existing token rule. **Under the parallel flavor (S36, plan-s33b) the read is a pinned task**, so the call moves into that `@task{id}` body and the host spine gets `mapal_par_wait(@pin{id}_entries)` → `mapal_par_check` → `mapal_par_run_pinned` at the read's topo position — the same host thread, at the same point, but now a DAG node the bracketed work has an edge to |

...
| tile sites `TilePlan` (S25 rung 1; S26 rung 2 TI×TJ) | `MorphismId ⇀ TileSite` — affine-triple recognition, lane strides {0,1}, guard-free | exported `flow_ir::CategoryIr::tile_plan` (`algo.rs:510`); consumed by `flow-backend-llvm` (`func.rs:emit_tiled_map` + `emit_tiled_map_blocked`); cuda consumption standing (S27 item 6) | deduced (recomputed, never stored) |

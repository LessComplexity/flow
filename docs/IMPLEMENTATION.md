...
| `emit` examples | file → lex→parse→lower → `.ll` / `.cu` text (dev tools — the `flow build` embryo) | `crates/flow-backend-{llvm,cuda}/examples/emit.rs` | built (S16) |

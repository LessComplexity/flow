...
| flow-rt | `crates/flow-rt/` — shared runtime seam (ADR-0020; owned by backend-llvm per its DESIGN §1): 7 `flow_print_*` externs (Rust `Display` = interp render parity) + `flow_trap` → exit 101; **S24: the parallel scheduler (11 `flow_par_*` externs — work-stealing pool, rank-seeded static schedule, watermark waits, CAS-min trap flag, registration-time pinning; `FLOW_PAR` env; std-only)**; **S25: cgroup-quota-aware width (`cpu.max` v2 / cfs v1 parsers; `FLOW_PAR` override absolute) + `flow_perf_begin`/`flow_perf_end` compute-timer externs (pool-warm then `Instant`; `FLOW_PERF total ms=` stdout grammar)**; `staticlib`+`rlib` | [DESIGN](components/backend-llvm/DESIGN.md) §1 + [plan-parallel-orchestrator](components/backend-llvm/plans/plan-parallel-orchestrator.md) | `crates/flow-rt/src/lib.rs` | built |
...
| task parallelism `PathPlan` (S24, plan-parallel-orchestrator) | `FuncId → PathPlan` — the task-DAG (fused/parallel/split tasks, speculate-and-order watermarks) | exported `flow_ir::CategoryIr::path_plan` (`algo.rs:946`); consumed by `flow-backend-llvm` (parallel `flow_main`) + `flow-rt` (work-stealing scheduler) | deduced (recomputed, never stored) |
| guard elision `BoundsProof` (S20c) | `FuncId → BoundsProof` — per-`Index` proven-in-bounds | exported `flow_ir::CategoryIr::bounds_proof` (`algo.rs:2008`); consumed by `tile_plan` (T4 precondition) + llvm/cuda guard elision | deduced (recomputed, never stored) |
| tile sites `TilePlan` (S25 rung 1; S26 rung 2 TI×TJ; S27 rung 3 packing; S28 fold k-split + window/conv rungs) | `MorphismId ⇀ TileSite` — affine-triple recognition, lane strides {0,1}, guard-free; S28: `TileRead.ksplit? : TileRead → TileKSplit` — fold-body `(k÷div, k%div)` derived axes (XOR raw `k`, `depth % div == 0`) | exported `flow_ir::CategoryIr::tile_plan` (`algo.rs:510`); consumed by `flow-backend-llvm` (`func.rs:emit_tiled_map` + `emit_tiled_map_blocked` + `emit_tiled_map_blocked_1d` + `emit_tiled_map_conv`; gates `packing_site`/`window1d_site`/`conv_site`); cuda consumption standing (S27 item 6) | deduced (recomputed, never stored) |
| last-use `LastUsePlan` | `FuncId → LastUsePlan` — dead-after per object | exported `flow_ir::CategoryIr::last_use_plan` (`algo.rs:1747`); consumed by llvm in-place `Update` (`llvm/func.rs:1861`) + cuda back-edge freeing/arena coloring (`cuda/func.rs:393`) | deduced (recomputed, never stored) |

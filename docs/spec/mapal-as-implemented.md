...
- **Core semantics is a gated Phi** (plan-s39): `Phi : (T × T × Bool) → T`, and the condition **gates** the arms — an arm's exclusive work runs only if the condition selects that arm. Both arms are still *compiled*; only *running* is conditional. Arms must be **pure**, and purity is not enough on its own: `Div`/`Mod`/`Index`/`Update` are pure but **partial**, so an ungated arm could trap on a path never taken (`(1 > 0) -> { -true-> 42; -false-> 7 / 0; }` used to exit 101). `mapal_ir::guard_plan` deduces each arm's exclusive work and whether it can trap; a site whose arms are cheap and total keeps today's `select` (gating and computing are then the same work). An effectful arm is L1404 (honest coproduct lowering is Core+1), `-> ret` inside an arm is L1405, and assigning an enclosing `mut` inside an arm is L1408 (the rebind would apply unconditionally).

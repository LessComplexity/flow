...
| 6 | `docs/spec/flow-language-design.docx` | historical |
...
Flow-Core (HANDOFF §4, ADR-0001) is the frozen implementation scope through M5. This section describes it as realized in the workspace today: `flow-syntax` → `flow-lower` → `flow-ir` (sealed + validated) → `flow-check` → `flow-rewrite` → `flow-interp` (the oracle) and `flow-backend-llvm`. Anything outside Core is rejected with a named diagnostic (§3), never silently accepted.
...
Meaning: Elgot / least-fixpoint iteration of a step `U → B ⊕ U` in the Kleisli category of the divergence monad (E1 / ADR-0002; the total core of Flow-Cat has no trace). Divergence is a **defined outcome** (`Diverged` under fuel), never a hang.
...
- `x -> { -> a; -> b; }` is a fanout of **pure** branches with an implicit join; branch bindings escape to the enclosing scope (the `fanout.flow` idiom). Parallelism is the default for structurally independent pure morphisms (user-guide §5; category-ir §9.5).
...
3. **E3 is vacuous-by-proof.** The memory guarantee (no use-after-free / double-free / leak / race) is scoped to the first-order non-cyclic core (ADR-0004) — and that scope currently contains **zero heap operations**: the IR omits the `Alloc`/`Load`/`Store`/`Free` quartet (ADR-0013), and `Update` is value semantics on stack-resident fixed arrays. `flow-check` implements no E3 code; the guarantee holds vacuously. The reopen trigger is pinned: the first heap-op ADR (dynamic arrays being the expected one).
...
|---|---|
| The formal frame — Flow-Cat, trace, functors | `category-ir.md` §2/§4/§8, with the E1 markers applied |
...
| The acceptance surface | `examples/*.flow` — ten Core programs; `vector.flow` is the aspirational-dialect exhibit, not Core |

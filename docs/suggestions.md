...
| 8 | backend-cuda | §5 deduce-don't-store / §4.5 Law 1 | Remaining headroom: region-v2 emission (#0 — the top item; `inline` pass in flight S20), by-value device products (#1), back-edge freeing (#2 — W3 in flight), batched trap checks (#3), scalar forwarding (#4 — subsumed by regions), grid-stride/geometry (#5), pinned memory (#6), CDP (#7), nvcc `-O3` row (#8), SoA/tiling (#9), parallel `Fold` (#10 — oracle ADR), NVRTC (#11), arena v1.1 last-use coloring (#18a) | **Open (S20)** — S20 discharged: captures (ADR-0027, S18), kernel dedup (#17), dead twins (#12), const-divisor elision (#13), trap-param trim (#14), arenas v1.0 (#18), kernel-time instrumentation (#19a) | [backend-cuda](components/backend-cuda/suggestions.md) |
